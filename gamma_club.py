...
class GCLUB(LinUCB_IND):
    # random_init: use random initialization or not
    def __init__(self, nu, d, T, k, seed, seeds, gamma=1, edge_probability = 1):

        #TODO: determine what is d here
        #TODO: read about graph structure through networkx package (what is edge probability ?)
        '''
        nu : number of users
        d : 
        T : number of rounds
        ...
        super(GCLUB, self).__init__(nu, d, T, gamma)
        self.nu = nu
    ...
    def store_info(self, i, x, y, t, r, br):
        super(GCLUB, self).store_info(i, x, y, t, r, br)

        c = self.cluster_inds[i]

        for ik, item_vec in enumerate(x):
            self.clusters[c].S += np.outer((self.gamma ** ik) * item_vec, (self.gamma ** ik) * item_vec)
        self.clusters[c].b += (self.gamma ** (2*(len(x) - 1))) * y * x[-1] if len(x) else 0
        self.clusters[c].N += 1
